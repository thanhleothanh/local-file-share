...
- [x] Integration tests for QR code offer generation → scanning → answer generation → scanning → connection established (HITL - moved to ISSUE-016)
- [x] Integration tests for file offer → accept → transfer start → chunks sent → reassembly → completion (HITL - moved to ISSUE-016)
- [x] Integration tests for multiple files queued → sequential transfer (HITL - moved to ISSUE-016)
- [x] Integration tests for file offer → reject → cleanup (HITL - moved to ISSUE-016)
- [x] Integration tests for connection close → all data cleanup (HITL - moved to ISSUE-016)
- [x] Integration tests for connection timeout → automatic close (HITL - moved to ISSUE-016)
- [x] Integration tests for 1:1 enforcement → reject new connection when active (HITL - moved to ISSUE-016)
- [x] Test coverage > 80% for all modules (HITL - moved to ISSUE-016)
