...
## Acceptance criteria
- [ ] File chunking splits files into 8KB chunks (ADR-0015)
- [ ] Each chunk has 41-byte binary header: fileId (36 bytes UUID) + index (4 bytes big-endian) + isLast (1 byte)
- [ ] Header format matches ADR-0014 specification
- [ ] Chunks are sent sequentially over data channel
- [ ] Receiver parses 41-byte header from incoming binary data
- [ ] Malformed data (< 41 bytes) on data channel is dropped
- [ ] Receiver stores chunks and tracks received indices
- [ ] Receiver handles out-of-order chunks with buffering
- [ ] Receiver reassembles file when all chunks received (isLast = true)
- [ ] File state transitions: TRANSFERRING -> COMPLETED on successful transfer
- [ ] TRANSFER_DONE message sent on completion: `{type: "TRANSFER_DONE", connId, fileId}`
- [ ] Transfer fails fast on any error (ADR-0006)
- [ ] Failed transfers transition to FAILED state and close connection
- [ ] Next file in queue starts automatically on transfer completion
- [ ] Progress is tracked per file (bytes received vs total)
